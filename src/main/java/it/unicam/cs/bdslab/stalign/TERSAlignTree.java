 ...
 * tertiary structure that may contain any kind of pseudo knot. It is used to
...
public class TERSAlignTree {
    ...
    /**
     * Initializes a new structural tree builder for the secondary structure given as parameter
    ...
    public TERSAlignTree(SecondaryStructure secondaryStructure){
        ...
        this.sequenceLength = this.secondaryStructure.getSecStrucState().size();
        this.structuralTree = null;
    }
    ...
    public TertiaryStructure getTertiaryStructure(){
        if(tertiaryStructure == null)
            throw new NullPointerException("Tertiary structure not initialized");
    ...
    private void detectZeroIntervals(int[] c, ArrayList<Interval> zi, int l, int r) {
        ...
        do {
            ...
            while (c[i] == 0)
                i++;
            // determine the stop of the zero interval, if start == stop, the zero interval
            // is empty
            int stop = i;
    ...
    private void initp(ArrayList<Integer>[] p){
        ArrayList<Pair<Integer>>bondList = this.tertiaryStructure.getBondList();
        if(bondList.size() == 0) {
            System.err.println("No bonds detected with the current threshold, can't generate the associated tree");
            System.exit(1);
        }
